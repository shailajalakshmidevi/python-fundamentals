...
## WAP to Count Digits, Vowels, and Consonants in a String
...
#         if string[i] in 'aeioiAEIOU':
#             Count_Vowel += 1
#         else:
#             Count_Consonants += 1
#     i += 1
# print(Count_Digit, Count_Vowel, Count_Consonants)

# wap to get the maximum and minimum from the tuple having only integer
tuple_ = (34, 56, 2424, 1, 9)
# max = 2424, min = 1

maximum_number = tuple_[0]
minimum_number = tuple_[0]

i = 0
while i < len(tuple_):
    if tuple_[i] > maximum_number:
        maximum_number = tuple_[i]
    elif tuple_[i] < minimum_number:
        minimum_number = tuple_[i]
    i += 1
print(minimum_number, maximum_number)
